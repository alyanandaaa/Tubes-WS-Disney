...
<h1 class="wow animated bounceInLeft">Tv Shows</h1>
</div>

<div class="sec-title text-center">
<form action="t.php" method="post" class="wow animated bounceInDown">
  <center><div class="form-group">
    <select class="form-control" id="exampleFormControlSelect1" name="judul" style="width: 60%;">
  <option value="all">--What's your favorite tv shows?--</option>
  <?php 
  $result = $sparql->query(
    'SELECT distinct ?id ?title ?judul{'.
    ' ?id dbo:network dbr:Disney_Channel.'.
    ' ?id dbp:title ?title.'.
    ' ?title foaf:name ?judul.'.
    '} ORDER BY ASC (?judul)'
  );
...
<div class="sec-title text-center">
<p>Jumlah shows: <?= $result->numRows() ?></p>
</div>
